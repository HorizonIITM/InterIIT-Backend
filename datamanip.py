...
class dataCleaner:
    ...
    def __byteIndexDict(self, path: str, lines: list, byteC: list, labelC: list):
        ...
        if self.hasInfo[0] and self.hasInfo[1]:
            with open(path, "r") as f:
                ...
                def isValid(line):
                    return (
                        line[byteC[0] : byteC[1]] != 7 * " "
                        and line[labelC[0] : labelC[1]] != 3 * "-"
                    )

                descLines = f.readlines()[lines[0] : lines[1]]
                byteIndex = [
                    [int(byt) for byt in line[byteC[0] : byteC[1]].split("-")]
                    for line in descLines
                    if isValid(line)
                ]
                dataLabel = [
                    line[labelC[0] : labelC[1]].strip()
                    for line in descLines
                    if isValid(line)
                ]
                byteDict = {}
    ...
    def makeCatalog(self):
        ...
        """
        self.__byteIndexDict(
            self.readme, self.readmeLines, self.readmeBytes, self.readmeLabel
        )
    ...
    def getQueryInfo(self, n: int):
        ...
        return {"identifier": identifier, "bibcodes": bibcodes}
    ...
    def makeObsCatalog(self):
        ...
        for _ in tqdm(range(len(self.ObsCatalog))):
            ...
            if queryText[0] == "!" or queryText.split("\n")[5][0] == "N":
                defIDS.append(self.ObsCatalog.iloc[_][6])
            elif queryText.split("\n\n")[2][0] == "O":
                defIDS.append(queryText.split("\n\n")[2].split("---")[0][7:].strip())
    ...
    def makeBibCatalog(self):
        ...
        with open(self.bibCat, "r", encoding="utf-8") as f:
            ...
            for paper in papers:
                ...
                fields[3] = (
                    fields[3].strip().replace('URL: <a href="', "")[:-26].strip()
                )
